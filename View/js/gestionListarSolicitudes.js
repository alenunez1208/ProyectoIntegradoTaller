var oCapaListado= document.getElementById("listadoCitas");
...
function respuestaListadoSolicitudesPendientes()
{
	...
	if (oAjax.readyState == 4 && oAjax.status == 200) {
		...
		for ( var i=0;i<oFilas.length;i++){
			...
			oCelda.appendChild(oTexto);
			oFila.innerHTML+="<td><input type='submit' class='btn btn-success' name='btnAceptarCita' id='btnAceptarCita' value='✓'> &nbsp <input type='submit' class='btn btn-danger' name='btnRechazarCita' id='btnRechazarCita' value='☓'></td>";
		}
